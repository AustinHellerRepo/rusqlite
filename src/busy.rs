...
use crate::{ffi, SetPointer, TakePointer};
...
impl Connection {
    ...
    pub fn busy_handler(&self, callback: Option<fn(i32) -> bool>) -> Result<()> {
        ...
        let sqlite_instance = c.db.take_mut_pointer();
        let r = match callback {
            Some(f) => unsafe {
                ffi::sqlite3_busy_handler(sqlite_instance, Some(busy_handler_callback), f as *mut c_void)
            },
            None => unsafe { ffi::sqlite3_busy_handler(sqlite_instance, None, ptr::null_mut()) },
        };
        c.db.set_mut_pointer(sqlite_instance);
        c.decode_result(r)
    }
}
